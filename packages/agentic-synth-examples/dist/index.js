...
import { AgenticSynth as AgenticSynth6 } from "@ruvector/agentic-synth";
var colors = {
  reset: "\x1B[0m",
  bright: "\x1B[1m",
  dim: "\x1B[2m",
...
`);
  ...
  async initializeGenerators(apiKeys) {
    ...
    for (const modelConfig of this.models) {
      ...
      try {
        generators[modelConfig.name] = new AgenticSynth6({
...
import { AgenticSynth as AgenticSynth7 } from "@ruvector/agentic-synth";
...
`);
    ...
    for (const modelKey of this.config.models) {
      ...
      try {
        this.generators[modelKey] = new AgenticSynth7({
